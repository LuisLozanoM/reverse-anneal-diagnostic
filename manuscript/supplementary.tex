...
The Zephyr-topology processor was operated under the solver identifier \texttt{Advantage2\_system1.13} for the Phase 3 and Phase 4 main-paper sweeps (Figs.~2, 3 and~5), and under the renamed identifier \texttt{Advantage2\_system1} (same physical hardware; qubit 4374 removed from the working graph) for the Phase 6 thermal-marginal campaign (Fig.~4); see main text Methods for the chronology.
...
2026-04-10 after qubit 4374 was flagged out of spec; the Phase 3/4
main-paper sweeps used the original identifier and the Phase 6 sweep
...
\midrule
Solver (Phase 3/4)       & \texttt{Advantage2\_system1.13} & \texttt{Advantage\_system6.4} \\
Solver (Phase 6)         & \texttt{Advantage2\_system1}    & \texttt{Advantage\_system6.4} \\
Topology                 & Zephyr                          & Pegasus \\
Active qubits            & 4{,}579                         & 5{,}612 \\
Reference energy scale (1D-chain critical point) & 2.308\,GHz                      & 1.281\,GHz \\
$\beta_\mathrm{eff}$ at $s_p{=}0.4$ (Phase 3/4) & $6.93\pm 0.20$ & $4.46\pm 0.26$ \\
$\beta_\mathrm{eff}$ at $s_p{=}0.4$ (Phase 6 main) & $7.219\pm 0.063$ & $4.289\pm 0.294$ \\
$\beta_\mathrm{eff}$ at $s_p{=}0.4$ (Phase 6 frustrated pilot) & $7.331\pm 0.198$ & --- \\
...
Legacy Phase~3 and Phase~4 submissions were programmed through the minor-embedding composite with the SDK default \texttt{auto\_scale=True}, which applies a uniform linear rescaling of the embedded problem to fit within the solver's programmable range.
...
To further check which main-text conclusions are robust to the submission path, we regenerated the Phase~4 $\lambda$ sweep and $|E|$ sweep with \texttt{auto\_scale=False} and paired them against \texttt{auto\_scale=True} arms on the same physical qubits.
...
\paragraph{Phase~7 mixed-frustration re-verification at \texttt{auto\_scale=False}.}
The Phase~7 working point (mixed frustration with $p_S=0.5$, $W=1$, $|E|=8$, $|S|=4$, $\lambda=0.5$, $s_p=0.4$, $t_p=100\,\mu$s) underpins the temperature-gap ratios of main-text Fig.~6.
...
\begin{table}[h]
\centering
\caption{Phase~7 mixed-frustration re-verification.  Sign seeds 100--109 at $p_S=0.5$, $W=1$, $|E|=8$, $|S|=4$, $\lambda=0.5$, $s_p=0.4$, \texttt{chain\_strength}$=2$.  Each cell is $\mathcal{M}$ pooled over three initial subsystem states (3$\times$2000 reads).}
...
\subsection{Phase 3: Additional discovery scans}
\label{sec:phase3}
...
Table~\ref{tab:phase3} summarises the parameter ranges swept in this discovery phase; for each scan point we compare $P_S$ (computed over a fixed six-qubit subgraph) across at least two initial states and compute the memory order parameter $\mathcal{M}$ as defined in the main text.
The Phase~3 scans confirmed that initial-state dependence decreases monotonically with increasing pause time $t_p$ at fixed $s_p=0.4$, with crossover kinetics on the order of a few microseconds; that decreasing $s_p$ (deeper pause, stronger transverse field) drives relaxation at shorter $t_p$; and that adding quenched disorder preserves initial-state memory above a threshold $W_c \approx 1.5$.
These findings motivated the upgrade to the subsystem--bath geometry in Phase~4.

\begin{table}[h]
\centering
\caption{Phase 3 parameter scans (discovery phase, single-partition problems).}
\label{tab:phase3}
\small
\begin{tabular}{llll}
\toprule
...
\subsection{Phase 4: Full subsystem-bath scan grids}
\label{sec:phase4}

Phase~4 introduces the explicit $S$/$E$ partition used throughout the main text: a connected six-qubit subsystem $S$ coupled to an environment $E$ of up to 50 qubits through boundary couplers of programmable strength $\lambda$.
...
Each Phase 4 data point is computed from at least three distinct initial states of $S$ (all-up, all-down, random) with 1{,}000--2{,}000 reverse-anneal reads per initial state, followed by bootstrap resampling (200 resamples) to estimate uncertainties.

\begin{table}[h]
\centering
\caption{Phase 4 subsystem--bath scans.}
\label{tab:phase4}
\small
\begin{tabular}{lll}
\toprule
...
The Phase 3 and Phase 4 main-paper experiments use logical-to-physical minor embedding to map the random 3-regular Ising problem onto the Zephyr or Pegasus hardware graph.
...
As a direct consistency check, the Phase 6 thermal-marginal campaign of Fig.~4 was performed on native Zephyr subgraphs (Phase 2 native, Phase 3A scaled sweeps, Phase 3B bath ensemble; Sec.~\ref{sec:phase6_inventory}), so that each logical qubit corresponds to a single physical qubit and no minor embedding is required.
The relaxation and thermal-marginal conclusions reported for the Phase 6 native-graph sub-experiments are consistent with the embedded Phase 3/4 results, and therefore constrain the magnitude of embedding-induced systematics.
...
We repeated selected Phase 4 conditions (the $|E|$ sweep at the relaxed point, and the $(\lambda, W)$ grid at $\lambda = 0.5$) across multiple calibration epochs on Advantage2 and verified that the memory order parameter agrees with the original measurements to within the bootstrap uncertainty in each case.
...
A future dedicated calibration campaign could probe whether a non-zero \texttt{readout\_thermalization} changes the measured crossovers at the level of 1--2\% bootstrap uncertainty; the Phase 6 Gibbs-fit analysis (Sec.~\ref{sec:phase6}) independently establishes that the present setting is sufficient for shot-noise-level thermal-marginal agreement on the relaxed subset.
...
A dedicated gauge-averaging control, in which each reverse-anneal job is repeated under several random gauge choices before averaging, was performed on three representative Phase 6 test cases (Sec.~\ref{sec:phase6_robustness}).
...
\section{Phase 6: Effective thermal marginal campaign}
\label{sec:phase6}

This section gives the full details of the Phase 6 Gibbs-fit campaign
summarised in the main-text Effective thermal marginal subsection.
All analyses draw on the per-condition JSON artefacts in
\texttt{data/raw/phase6\_gibbs/*/} and are reproducible from
\texttt{scripts/phase6\_gibbs\_campaign.py} plus the reanalysis driver
\texttt{scripts/reanalyze\_phase6\_with\_correct\_ab.py}.
...
(one per solver); the nearest-sample values at $s_p=0.4$ are stored in
\texttt{data/raw/phase6\_gibbs/phase1\_calibration/schedules/schedule\_at\_sp0.4.json}.

\begin{table}[h]
\centering
\caption{Per-QPU anneal schedule coefficients at the Phase 6 pause
...
The first run of the Phase 6 campaign used an approximate
...
The Phase 6 campaign re-measured $\beta_\mathrm{eff}$ under the same
single-qubit probe protocol used for the main paper (Sec.~\ref{sec:qpu}),
after the Advantage2 solver rename.  For the main Phase 6 sweep
(Phase~2 native and random-regular, Phase~3A three scales,
Phase~3B bath-ensemble, Phase~3C disorder sweep) we use
...
in \texttt{data/raw/phase6\_gibbs/phase1\_calibration/}.
...
\begin{table}[h]
\centering
\caption{Phase 6 sub-experiment inventory.  Every sub-experiment uses
...
\midrule
Phase 2 exact (native) & 4 & $\{6,8,10,12\}$ & $\{0.2,0.3,0.5\}$, $W\in\{0,0.5,1\}$ & 84 & native Zephyr \\
Phase 2 exact (3-reg) & 4 & $\{6,8,10,12\}$ & same & 84 & random 3-regular \\
Phase 3A scale$=0.35$ & 4 & $\{6,8,10,12\}$ & same, $(h,J)\times0.35$ & 84 & native Zephyr \\
Phase 3A scale$=0.5$ & 4 & $\{6,8,10,12\}$ & same, $(h,J)\times0.5$ & 84 & native Zephyr \\
Phase 3A scale$=0.75$ & 4 & $\{6,8,10,12\}$ & same, $(h,J)\times0.75$ & 84 & native Zephyr \\
Phase 3B bath unscaled & 6 & $56$ & $|E|=50$, $\lambda=0.5$ & 7 & native Zephyr \\
Phase 3B bath scale$=0.5$ & 6 & $56$ & same, $(h,J)\times0.5$ & 7 & native Zephyr \\
Phase 3C disorder sweep & 4 & $12$ & $\lambda=0.5$, $W\in\{0,0.25,0.5,0.75,1,1.25\}$ (10 seeds) & 60 & random 3-regular \\
Phase 3C cross-QPU & 4 & $12$ & same & 60 & 3-regular (System 6.4) \\
Phase 6 frustrated pilot & 4 & $12$ & $\lambda=0.5$, EA $\pm J$ flips, 5 seeds $\times$ 3 $W$ & 15 & random 3-regular \\
Phase 6 robustness & 4 & $\{8,12\}$ & 3 test cases $\times$ 3 return-ramps $+$ 4 gauges & 21 & native / 3-regular \\
\bottomrule
\end{tabular}}
\end{table}

The $494$-condition Phase 6 main sweep on Advantage2 referenced in the
...
Phase 6 sub-experiment.  \emph{classical cond} uses
...
\midrule
Phase 2 native   & $10/84$ & $4\!\times\!10^{-4}$ & $2.8\!\times\!10^{-3}$ & $0.028$ & $0.056$ & $0.031$ \\
Phase 2 3-reg    & $18/84$ & $2\!\times\!10^{-4}$ & $1.7\!\times\!10^{-2}$ & $0.021$ & $0.448$ & $0.068$ \\
Phase 3A sc$=0.35$ & $12/84$ & $1.3\!\times\!10^{-2}$ & $0.941$ & $0.222$ & $0.541$ & $0.260$ \\
Phase 3A sc$=0.5$  & $10/84$ & $5.9\!\times\!10^{-3}$ & $0.967$ & $0.109$ & $0.357$ & $0.142$ \\
Phase 3A sc$=0.75$ & $9/84$  & $1.8\!\times\!10^{-3}$ & $1.6\!\times\!10^{-2}$ & $0.047$ & $0.093$ & $0.047$ \\
Phase 3B unscaled  & $5/7$   & $<\!3\!\times\!10^{-4}$ & $5.0\!\times\!10^{-4}$ & --- & --- & --- \\
Phase 3B sc$=0.5$  & $6/7$   & $<\!1\!\times\!10^{-4}$ & $2.6\!\times\!10^{-4}$ & --- & --- & --- \\
Phase 3C Adv.2     & $43/60$ & $\sim\!10^{-6}$        & $1.6\!\times\!10^{-2}$ & $0.0145$ & $0.0272$ & $0.504$ \\
Phase 3C Sys~6.4   & $22/60$ & $5.2\!\times\!10^{-3}$ & $2.9\!\times\!10^{-2}$ & $0.0146$ & $0.0284$ & $0.173$ \\
Phase 6 frustrated & $2/15$  & $0.320$ & $0.639$ & $0.029$ & $0.045$ & $0.025$ \\
\bottomrule
\end{tabular}}
\end{table}

Phase 3B does not report quantum-target columns because the
...
below $10^{-3}$ on all $11$ relaxed Phase 3B conditions and is used
...
conditional quantum reduced Gibbs target is tight on Phase 3C
(max $\approx 0.029$ on both QPUs) but progressively looser as the
Hamiltonian is scaled down, with the Phase 3A scale=$0.35$ row having
...
$s$: at Phase 3A scale $s$, the effective transverse-to-longitudinal
...
\subsection{Phase 3C per-$W$ disorder statistics}
...
\begin{table}[h]
\centering
\caption{Phase 3C disorder sweep, $N=12$, $|S|=4$, $\lambda=0.5$,
...
Three relaxed Phase 6 main-sweep conditions have classical conditional
...
target at $D_\mathrm{TV}>0.05$ in the 494-condition Phase 6 main
...
parameters as Phase 3C, then flipped the sign of each coupler
...
cases from Phase 3C and Phase 3A at return-ramp durations
...
(case, ramp, gauge) combination are stored in
\texttt{data/raw/phase6\_gibbs/phase6\_robustness/comparison.json}.

\subsection{Readout confusion-matrix calibration}
\label{sec:phase6_readout}

The $|S|=6$ Phase 6 readout confusion matrix was measured by preparing
...
the Phase 6 analysis, and the multinomial sampling floor of
...
and the raw calibration reads are stored in
\texttt{data/raw/phase6\_gibbs/phase1\_readout/}.

\section{Phase 7: Barrier crossing under mixed frustration}
\label{sec:phase7}

This section provides full details for the Phase 7 experiments
