...
nodelist.  For the Phase 6 main sweep on Advantage2
(Phase~2, Phase~3A, Phase~3B and Phase~3C sub-experiments), taken after
...
respectively.  Across the full $494$-condition Phase 6 main sweep on
Advantage2 (Phase 2 native Zephyr, Phase 2 random 3-regular, Phase 3A
at three Hamiltonian scales, Phase 3B bath-ensemble, and Phase 3C
disorder sub-experiments), $113/494$ conditions satisfy the relaxation
...
The conditional quantum reduced Gibbs target is tight in the Phase 3C
disorder sweep but not uniformly across all scaled ferromagnetic
conditions.  On the Phase 3C sweep at $N=12$, $|S|=4$, $\lambda=0.5$ on
...
concentrated in the Phase 3A scaled sweeps.  Scaling the problem
...
seen by the reduced Gibbs state is $A/(sB)$; at Phase 3A scale $0.35$
...
$D_\mathrm{TV}\approx 0.5$ in the Phase 3C Advantage2 sweep), which is
...
Three relaxed conditions in the 494-instance Phase 6 main sweep fail
...
Among the $381$ Phase 6 main-sweep conditions with $\mathcal{M}>0.05$,
...
\textbf{a,}~Fraction of relaxed disorder realisations ($\mathcal{M}\le 0.05$) in the 10-seed Phase 3C disorder sweep ($N=12$, $|S|=4$, $\lambda=0.5$, 3-regular logical graph, Advantage2).  Relaxation fraction decreases from $10/10$ at $W=0$ to $4/10$ at $W=1.25$.
...
\textbf{c,}~Scatter of $D_\mathrm{TV}$ vs $\mathcal{M}$ across all Phase 6 conditions.  Red: relaxed ($\mathcal{M}\le 0.05$); grey: memory-retaining.  Two top-left red outliers correspond to the same $\lambda=0.2$, $N=8$, $W=1$, seed~42 native Zephyr instance at Hamiltonian scales $0.35$ and $0.5$ (dynamical trapping, discussed in the body).  The memory and thermal-marginal criteria identify the same condition set apart from this trapping pair and one borderline $D_\mathrm{TV}=0.051$ miss.}
