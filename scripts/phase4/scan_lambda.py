"""Phase 4 scan: S-E coupling strength lambda sweep."""
...
def run_scan(config_path: str, output_dir: str, qpu: str = "primary"):
    ...
    scan = config["scans"]["scan_lambda"]
    defaults = config["defaults"]
    qpu_config = config["qpus"][qpu]
    out = Path(output_dir) / "phase4" / "scan_lambda" / qpu
    ...
    for lam in lambda_values:
        ...
        for name, init_state in initial_states.items():
            result = run_reverse_anneal(
                ...
                label=f"scan_lam_{name}_lam{lam}",
            )
            save_raw_results(
                samples=result["samples"], energies=result["energies"],
                metadata=result["metadata"],
                path=out / f"lam{lam}_{name}.h5",
            )
        ...
        print(f"  lambda={lam}: memory={memory:.4f}")

    summary = {
        "scan": "lambda", "qpu": qpu, "solver": qpu_config["solver"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 4: lambda scan")
