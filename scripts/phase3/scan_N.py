"""Phase 3 scan: system size N sweep at fixed s_p, t_p, W=0."""
    ...
    for n_qubits in N_values:
        ...
        for name, init_state in initial_states.items():
            result = run_reverse_anneal(
                ...
                label=f"scan_N_{name}_N{n_qubits}",
            )
            save_raw_results(
                samples=result["samples"], energies=result["energies"],
                metadata=result["metadata"],
                path=out / f"N{n_qubits}_{name}.h5",
            )
        ...
        results.append({"n_qubits": n_qubits, "memory_order_param": memory, "tvd_pairs": tvd_pairs})
        print(f"  N={n_qubits}: memory={memory:.4f}")

    summary = {
        "scan": "N", "qpu": qpu, "solver": qpu_config["solver"],
        "s_p": s_p, "t_hold": t_hold, "disorder_W": W,
        "results": results,
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 3: N scan")
